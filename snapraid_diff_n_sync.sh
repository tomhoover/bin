#!/usr/bin/env bash
# shellcheck disable=SC2004,SC2129
...
PARITY_FILE=$(grep snapraid.parity /etc/snapraid.conf | head -n 1 | cut -d " " -f2)
...
if [ "$DEL_COUNT" -gt 0 ] || [ "$ADD_COUNT" -gt 0 ] || [ "$MOVE_COUNT" -gt 0 ] || [ "$UPDATE_COUNT" -gt 0 ]; then
    ...
    echo "WARNING - Deleted files ($DEL_COUNT) exceeded threshold ($DEL_THRESHOLD). Check $TMP_OUTPUT for details. NOT proceeding with sync job."
        ...
        # cleanup email body
        grep -E -v "^[[:digit:]]+" $TMP_OUTPUT | /usr/bin/mutt -s "$EMAIL_SUBJECT_PREFIX Sync Job COMPLETED" -a $TMP_OUTPUT -- "$EMAIL_ADDRESS"
        ...
        if [ $RUN_SCRUB = "true" ]; then
            if [ $COUNTER -lt $SCRUB_FREQUENCY_IN_DAYS ]; then
               ...
               # cleanup email body
               grep -E -v "^[[:digit:]]+" $TMP_OUTPUT | /usr/bin/mutt -s "$EMAIL_SUBJECT_PREFIX Full Scrub COMPLETED" -a $TMP_OUTPUT -- "$EMAIL_ADDRESS"
